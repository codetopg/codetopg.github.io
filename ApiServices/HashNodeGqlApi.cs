...
namespace codeTopGBlazorWasm.ApiServices
{
    public class HashNodeGqlApi : IHashNodeGqlApi, IDisposable
    {
        ...
        {
            string query = "{user(username: \"codetopg\") {publication {posts(page: 0) {slug title brief coverImage dateUpdated }}}}";
        ...
        {
            string query = $"{{post(slug:\"{slug}\", hostname:\"\") {{ content title }} }}";
